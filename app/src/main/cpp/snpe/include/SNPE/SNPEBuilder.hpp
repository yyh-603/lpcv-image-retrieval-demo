 ...
 * @brief Sets custom performance profile
 *
 * @param[in] deferredInit, true implies deferredInit is enabled
 *
 * @return SNPE_SUCCESS upon successful setting of performance profile
 */
  Snpe_ErrorCode_t setDeferredInit(bool deferredInit){
    return Snpe_SNPEBuilder_SetDeferredInit(handle(), deferredInit);
  }
