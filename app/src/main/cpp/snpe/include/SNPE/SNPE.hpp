 ...
 */
  ...
  *
  * @return true on successful retrieval of spill-fill buffer size
  */
  bool getSpillFillBufferSize(uint64_t& size) {
    return SNPE_SUCCESS == Snpe_SNPE_GetSpillFillBufferSize(handle(), &size);
  }
  ...
  * @return true on successful retrieval of spill-fill buffer size
  */
  bool getWeightsBufferSize(uint64_t& size) {
    return SNPE_SUCCESS == Snpe_SNPE_GetWeightsBufferSize(handle(), &size);
  }
  ...
  *
  * @return true on successful retrieval of size of alignment
  */
  bool getBufferAlignment(uint32_t& size) {
    return SNPE_SUCCESS == Snpe_SNPE_GetBufferAlignment(handle(), &size);
  }
  ...
  */
  bool finalize(int32_t spillFillBufferFd, int32_t weightsBufferFd)
  {
    return SNPE_SUCCESS == Snpe_SNPE_Finalize(handle(), spillFillBufferFd, weightsBufferFd);
  }
